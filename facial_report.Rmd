...
Based on our model, we did posterior prediction about each factors, the output can be found in Table 7.
...
```{r fig.align='center', fig.cap="Plot for Multinomial Regression Model"}
...
After further consideration, we decided to build a multinomial model as well to quickly compare its results to our prior models. As seen in the Figure 9 of the confidence intervals above, superior facial site has been set as the baseline(vertical line) for comparisons. The multinomial model suggests that we do not have enough evidence to conclude any of these variables are significant different in predicting midface and superior facial trauma. In the prediction of mandible injuries and those classified as "other," we see a general trend of the interaction terms having a negative effect, and the individual variables having a positive effect as comparing to superior trauma sites. The only exception to this trend is the gun variable and the interaction term containing gunshot injuries. However, this dataset did not include many instances of gunshot injuries so the lack of data could have skewed this result. In addition, this multinomial plot suggests we do not have enough evidence to conclude that gender plays a significant role in the prediction of any area of facial trauma.


+ Results:

```{r fig.align="center", fig.cap="Prediction of probability of facial trauma sites in different injury mechanisms and different gender", fig.height=10, fig.width=7}
tabl<-cbind(cleaned_set[,c(5,7,12)],round(fitted(mul_site),2))
...
#tablsum$Gender <- factor(tablsum$Gender, levels=c("Male", "Female"))

###### visualization of prediction ######
ggplot(melt(tablsum,id.vars=c("Gender","Urban.Rural","Injury.Mech.category")))+geom_point()+aes(x=variable,y=value,color=Urban.Rural,group=Urban.Rural)+facet_wrap(~Injury.Mech.category+Gender, ncol=2)+coord_polar()+ geom_polygon(fill=NA)


####ignore injury type #####
#tablsum2 <- tablsum %>% group_by(Gender,Urban.Rural)
```

Table 9 and Table 10 show the numeric predicted probabilities of male and female patients on facial trauma sites with respect to patient's residencies in urban or rural and across different injury mechanisms. As we can see in those two table, there is no significant difference of the prediction probability in female and male. Across four trauma sites, there are higher prediction probabilities for midface facial trauma than other three trauma sites.   

In Figure 10, we compare the probability of getting hurt in four facial trauma sites in different residency and injury mechanisms.

From this plot, we can see:

+ We organized same injury mechanism with two different genders in the same row, there is no enough evidence to prove that two genders have significant different effect on trauma site prediction.
...
First of all, for Generalized Linear Mixed-Effects model(GLMMs), we see the random effects between "Urban.Rural" and "Injury.Mech.category" are neglectable small because we only have two categories for "Urban.Rural". Therefore, in Multinomial Regression model, we add an interaction between "Urban.Rural" and "Injury.Mech.category" and shows that almost all coefficients of interaction between the injury mechanisms and patients' residencies are negative except the injury mechanism gun for mandible trauma site.

In addition, GLMMs show us the prediction results of each facial trauma sites individually and somehow it is hard to find correlation between those four trauma sites. But the Multinomial Regression model combined four trauma site as one single outcome and set the minimal prediction result, which is the prediction probability for superior facial trauma site, as the baseline. As shown in Figure 9, we can see more clear and straight forward comparison and correlation between those facial trauma sites and the superior trauma site.  

Last but not the least, in Figure 7, which is the posterior predictive checks plot for GLMMs, shows many predicted zero for mandible, superior, and other facial trauma sites without giving detailed estimation information about how each predictor can have impacts on trauma site predictions. But in Figure 10, which is the prediction probability plot of facial trauma sites for urban and rural with respect to different injury mechanisms and genders, gives us a more comprehensive understanding about the relationship between trauma sites, residencies, patient's gender and injury mechanisms.
...
```{r}
kable(fixef(ml_mandible), "pipe", caption ="GLMMs: Fixed effects of Mandible")
kable(fixef(ml_superior), "pipe", caption="GLMMs: Fixed effects of superior")
kable(fixef(ml_midface), "pipe", caption = "GLMMs: Fixed effects of midface")
kable(fixef(ml_other), "pipe", caption = "GLMMs: Fixed Effects of other")
...
```{r}
library(car)
Anova(mul_site, type = 3) %>% kable("pipe", caption = "Coefficients of Multinomial Model")

```

```{r}
tablsum[which(tablsum$Gender=="Male   "),2:7] %>%
  kable("pipe", caption = "Mulinomial: Prediction of probability of facial trauma in Male")

tablsum[which(tablsum$Gender=="Female "),2:7] %>%
  kable("pipe", caption = "Multinomial: Prediction of probability of facial trauma in Female")
