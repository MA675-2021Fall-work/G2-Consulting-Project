...
Based on finding from our EDAs, we use the Generalized Linear Mixed-Effects Regression Model to estimate the mixed effects of getting facial trauma in one of four trauma sites using “Age” and “Gender” as the predictors, add “Urban.Rural” for the random slope effect, and use “Injury.Mech.catergory” as the random intercept. The function of the model is shown below:

$$
\begin{aligned}
stanglmer(“Trauma site” ~ as.numeric(Age) + Gender  + Injury.Mech.category \\ +
(1+Urban.Rural|Injury.Mech.category), family = binomial(link="logit"))
\end{aligned}
$$

By arranging all four fitted models into one plot, we can see the same model generates different results in different trauma sites. The mixed-effects have greater impacts on superior and midface trauma sites. One reason that can cause this discrepancy is that midface and superior have bigger sample sizes than mandible and other. More information can be found in the four fixed effect table of the models. We also need validation to further assess the effectiveness of this model.

Generalized Linear Mixed-Effects Regression Models are validated by posterior predictive checks and binned residual plots.  The posterior predictive checks plot displays the comparison distribution of the observed outcome of trauma site(y) and simulated outcome of trauma site(y_rep) in which both distributions match well in the pattern, but simulated y_rep show more predicted zero which means zero inflation occurred except for the midface. On the other hand, binned residual plots show that some points lie outside the confidence limits for the mandible site and there is a pattern in the plots for the superior facial trauma site. Therefore, the generalized linear mixed-effects model is not the best-fit in assessing the prediction of relationship between all four trauma sites and patients residencies. Other models should be taken into consideration. 
...
#summary(ml_mandible)
p1<- plot(ml_mandible, probs = 95)+ggtitle("mandible")
...
Secondly, for patients with the same injury mechanism and from the same residency, men have 16% more possibility to get hurt at superior and 10% at other facial sites compared with women. Similarly, women are easier to get hurt at mandible and midface, the differences are 12.5% and 7.5%.
...
Fourthly, for patients with the same gender and same injury mechanism, if the accidents occur in urban areas, patients have approximately 7%  more likely to get hurt at mandible face, and 5% or so less likely at superior face.
